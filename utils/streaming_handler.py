...
class StreamingHandler:
    ...
    async def handle_non_streaming_response(
        ...
        """Handle non-streaming response. Uses OBO headers first; on 403 retries with fallback_headers (SP)."""
        try:
            ...
            if response.status_code == 403 and fallback_headers:
                logger.warning("Serving endpoint 403 with OBO; retrying with SP")
                response = await request_handler.enqueue_request(url, fallback_headers, request_data)
