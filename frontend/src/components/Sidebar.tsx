...
const InfoIcon = styled.div`
  ...
  &:hover::after {
    content: "O histórico de conversas é temporário e será limpo periodicamente";
    position: absolute;
    left: 24px;
    top: 50%;
...
const ChatItem = styled.div<{ active: boolean }>`
  ...
  cursor: pointer;
  font-size: 15px;
  color: ${props => props.active ? 'var(--accent-current)' : 'var(--text-primary)'};
  background-color: ${props => props.active ? 'rgba(79, 70, 229, 0.08)' : 'transparent'};
  border-radius: 4px;
  height: 32px;
  display: flex;
  align-items: center;
  ...
  const groupedChats = useMemo(() => {
    ...
    chats.forEach(chat => {
      ...
      if (chatDate.getTime() === today.getTime()) {
        ...
        dateCategory = 'Últimos 7 dias';
      } else {
        dateCategory = 'Mais antigos';
      }
