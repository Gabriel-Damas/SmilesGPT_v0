  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const messageListener = (event: MessageEvent) => {
      try {
        ...
        if (data.type === 'status' && data.thinkingStatus) {
          onChunk({
        ...
        if (data.type === 'response.output_item.done') {
          ...
          if (data.item && data.item.content && data.item.content[0]) {
            ...
            // Always use the final content from the server as it's the authoritative response
            // Only fallback to accumulated content if final is empty
