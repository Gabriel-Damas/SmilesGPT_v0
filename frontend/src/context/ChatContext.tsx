  ...
  const sendMessage = async (content: string, includeHistory: boolean = true, displayContent?: string) => {
    ...
    try {
      ...
      await apiSendMessage(content, currentSessionId, includeHistory, currentEndpoint, (chunk) => {
        ...
        // Handle status updates (thinkingStatus from backend)
        if (chunk.thinkingStatus) {
          setMessages(prev => {
            const updated = prev.map((msg, index) => {
              // Update the thinking message with the new status
              if (index === prev.length - 1 && msg.message_id === thinkingMessage.message_id && msg.role === 'assistant') {
                return { 
                  ...msg, 
                  thinkingStatus: chunk.thinkingStatus,
                  isThinking: true
                };
              }
              return msg;
          ...
          return; // Don't process further if it's just a status update
        }
          ...
          const updated = prev.map((msg, index) => {
            ...
            if (index === prev.length - 1 && msg.message_id === thinkingMessage.message_id && msg.role === 'assistant') {
              return { 
                ...
                thinkingStatus: chunk.thinkingStatus || msg.thinkingStatus, // Preserve status if available
        ...
        // Only set loading to false when response is complete
        if (isComplete) {
          setLoading(false);
        }
      });
